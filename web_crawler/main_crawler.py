...
class NoticeCrawler:
    ...
    def run(self):
        ...
        for site in self.url_list:
            # 각 사이트별로 collected_data를 초기화합니다.
            self.collected_data = []

            self.process_site(site)

            # 현재 사이트에서 수집된 데이터로 데이터프레임을 생성합니다.
            site_collected_df = pd.DataFrame(self.collected_data)

            if not site_collected_df.empty:
                # FileUtils.save_dataframe_to_csv 함수가 내부적으로 파일 이름을 결정하도록 변경
                # self.csv_save_directory는 이제 저장될 기본 디렉토리 경로만 전달합니다.
                # FileUtils 내부에서 site 정보 등을 활용하여 고유한 파일명을 생성해야 합니다.
                FileUtils.save_dataframe_to_csv(site_collected_df, self.csv_save_directory)
                print(f"✅ '{site['description']}' 크롤링 완료. {len(self.collected_data)}개의 게시물 수집됨. CSV 파일 저장됨.")
            else:
                print(f"ℹ️ '{site['description']}'에서 수집된 게시물이 없습니다.")
