...
class AttachmentProcessor:
    ...
    """
    지정된 url에 있는 파일 다운로드를 수행하는 함수
    성공시, 파일경로와 파일이름을 튜플 형태로 반환한다
    """
    def download_file(self, url):
        """
        decode_filename, fully_unquote함수는
        파일 다운로드시, 각 학과 홈페이지마다 다른 인코딩 방식을 사용하여
        하나의 디코딩 방식 사용시 파일명이 깨지는 문제가 발생함.
        따라서, 이를 방지하기 위해 2가지 디코딩 방식 중 적합한 디코딩 방식을 사용하는 것을 지원한다.
        """
    ...
    def pdf_extractor(self, file_path: str) -> str:
        ...
        try:
            ...
            text = ""
            for page_num in range(min(5, len(doc))):  # 최대 2페이지까지
                page = doc[page_num]
                text += page.get_text()
            return text.strip()
        except Exception as e:
    ...
    def hwp_extractor(self, file_path: str) -> str:
        ...
        for section in sections:
            ...
            section_text = ""
            i = 0
            size = min(80000, len(unpacked_data))
            while i < size:
    ...
    """
    file_path를 받고 해당 파일의 확장자에 따라, 텍스트 추출 로직 실행후
    텍스트를 반환하는 함수
    """
    ...
    """
    url_list를 입력으로 받아 해당 url의 파일을 다운로드하고, 텍스트를 추출한 결과를 반환하는 함수
    """
    def process_attachments(self, url_list):
        ...
        for url in url_list:
            ...
            if file_path:
                ...
                if(len(text)>25000): ## csv 셀 깨짐 방지 코드 25000 길이 초과하는 문자열 발생시 25000번째까지만 문자열 취함
                    text = text[:25001]
                results[url] = text
...
if __name__ == "__main__":  
    ...
    image_url = "https://wwwk.kangwon.ac.kr/www/downloadBbsFile.do?atchmnflNo=106150&bbsNo=345&nttNo=178954&&pageUnit=10&key=1959&pageIndex=10"
